...
const LeftArrow = () => {
  ...
  return (
    <Typography onClick={() => scrollPrev()} className="right-arrow">
      <img width="40px" src={LeftArrowIcon} alt="right-arrow" />
    </Typography>
  );
};
...
const RightArrow = () => {
  ...
  return (
    <Typography onClick={() => scrollNext()} className="left-arrow">
      <img width="40px" src={RightArrowIcon} alt="right-arrow" />
    </Typography>
  );
};
...
const HorizontalScrollbar = ({ data, bodyPart, setBodyPart, isBodyParts }) => {
  return (
    <ScrollMenu clas LeftArrow={LeftArrow} RightArrow={RightArrow} > 
        {data.map((item)=>(
            <Box 
              key={item.id || item}
              itemId={item.id || item}
